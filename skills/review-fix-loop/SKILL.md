...
Resolve `scripts/review_loop_listener.py` relative to this `SKILL.md`. The first workflow action after role validation and path resolution is starting this command in an independent long-running background tool session:

```bash
python3 <listener_script> listen \
...
```bash
python3 <listener_script> verify \
  --state <absolute_listener_state_json> \
  --role <reviewer_or_fixer> \
  --watch <absolute_peer_file> \
  --max-age 5
